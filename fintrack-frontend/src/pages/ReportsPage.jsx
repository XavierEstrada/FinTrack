...
export default function ReportsPage() {
  return (
    <div className="space-y-4 md:space-y-5">
      ...
      >
        <div className="flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-600 dark:text-slate-300">
          <CalendarDays size={15} className="text-slate-400 shrink-0" />
          <span>01 Nov 2025 — 30 Abr 2026</span>
        </div>
        <button className="flex items-center gap-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-medium px-4 py-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
          <Download size={15} />
          Exportar CSV
        </button>
      </motion.div>
      ...
      >
        ...
        <div className="flex justify-between mt-1 px-4">
          {mockMonthlyData.map(d => (
            <span key={d.month} className="text-[11px] text-slate-400 dark:text-slate-500">{d.month}</span>
          ))}
        </div>
      </motion.div>
      ...
      >
        <div className="px-4 md:px-5 py-4 border-b border-slate-100 dark:border-slate-800">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Gastos del período seleccionado</p>
        </div>
          ...
          <table className="w-full text-sm min-w-[480px]">
            ...
            >
              {mockCategoryExpenses.map((c, i) => (
                <motion.tr key={c.name} variants={rowItem} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-4 md:px-5 py-3.5 text-right font-semibold text-slate-800 dark:text-slate-100">{formatCurrency(c.amount)}</td>
                  <td className="px-4 md:px-5 py-3.5 text-right text-slate-500 dark:text-slate-400">{Math.round((c.amount / totalExpenses) * 100)}%</td>
                  <td className="px-4 md:px-5 py-3.5">
