    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-10 w-full max-w-sm text-center shadow-sm">
          ...
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-50 mb-2">{t('auth.register.successTitle')}</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">{t('auth.register.successMessage')}</p>
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-violet-600 via-indigo-700 to-indigo-800 flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <div className="bg-white/10 rounded-2xl p-6 backdrop-blur-sm">
            <p className="text-white text-sm font-medium mb-3">{t('auth.register.heroIncluded')}</p>
            {[
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 bg-white dark:bg-slate-950 overflow-y-auto">
        <div className="w-full max-w-sm">
          ...
          <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-50 mb-1">{t('auth.register.title')}</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm mb-8">{t('auth.register.subtitle')}</p>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">{t('auth.register.fullName')}</label>
              <input {...register('fullName')} type="text" placeholder={t('auth.register.fullNamePlaceholder')} className={inputCls} />
              {errors.fullName && <p className="text-red-500 text-xs mt-1">{errors.fullName.message}</p>}
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">{t('auth.register.email')}</label>
              <input {...register('email')} type="email" placeholder={t('auth.register.emailPlaceholder')} className={inputCls} />
              {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email.message}</p>}
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">{t('auth.register.password')}</label>
                <div className="relative">
                ...
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">{t('auth.register.passwordHint')}</p>
                {errors.password && <p className="text-red-500 text-xs mt-1">{errors.password.message}</p>}
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">{t('auth.register.confirmPassword')}</label>
                <div className="relative">
