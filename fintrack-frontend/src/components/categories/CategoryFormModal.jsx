...
export const CUSTOM_COLORS = [
  ...
  { value: '#f43f5e', key: 'pink'   },
  { value: '#c026d3', key: 'fuchsia' },
  { value: '#0284c7', key: 'ocean'  },
  { value: '#16a34a', key: 'forest' },
  { value: '#a16207', key: 'gold'   },
]
...
export default function CategoryFormModal({ isOpen, onClose, category = null }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEditing ? t('categoryModal.editTitle') : t('categoryModal.newTitle')} size="sm">
      <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="flex items-center gap-2.5">
            {CUSTOM_COLORS.map(c => (
              <button
                key={c.value}
                type="button"
                title={t(`categoryModal.colors.${c.key}`)}
        ...
        <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
          ...
          <button type="submit" disabled={isPending}
            ...
            {isPending ? t('common.saving') : isEditing ? t('common.save') : t('categoryModal.createBtn')}
          </button>
        </div>
      </form>
    </Modal>
